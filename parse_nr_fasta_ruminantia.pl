...
{
	my $done=0;
	if (exists $ruminantia{$node})
	{
		if ($ruminantia{$node}==1)
		{
			$done=1;
		}
	}
...
if (open(OUT, ">$dir/ruminantia/ruminantia.txt"))
{
	print OUT qq!name\tid\truminantia\n!;
	foreach my $name (sort keys %names)
	{
		print OUT qq!$name\t$names{$name}\t$ruminantia{$names{$name}}\n!;
	}
	close(OUT);
}
...
if (open(IN, "$fasta"))
{
	if (open(OUT, ">$dir/ruminantia/$file_name-edited.fasta"))
	{
		open(OUT1, ">$dir/ruminantia/species_table.txt");
		open(LOG, ">$dir/ruminantia/parse_nr_fasta.log");
		open(OUT_NO, ">$dir/ruminantia/$file_name-edited-not-primate.fasta");
		open(OUT1_NO, ">$dir/ruminantia/species_table-not-primate.txt");
		open(LOG_NO, ">$dir/ruminantia/parse_nr_fasta-not-primate.log");
		my $gi_num_previous="";
		my $line_previous="";
		while (my $line = <IN>)
		{
			...
			if ($line =~ /^>gi\|(\d+)\|/)
			{
				...
				if ($line_previous=~/\w/)
				{
					...
					my @name_list_vert = ();
						...
						{
							if (exists $ruminantia{$names{$item_name}})
							{
								if ($ruminantia{$names{$item_name}}==1)
								{
						...
						{ 
						    if ($ruminantia==1) { print LOG qq!Error: $gi_num_previous $item_name\n!; } else { print LOG_NO qq!Error: $gi_num_previous $item_name\n!; } 
						}
                                        }
